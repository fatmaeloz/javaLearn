...
private static String kontrolChar7AzTernary(List<String> yemahhh) {
    return yemahhh.stream().allMatch(t -> t.length() <= 7) ?
            "Yemek menunun tum elemanlarinin uzunlugu 7 veya 7'den az karakterde" :
            "Yemek menunun en az bir elemaninin uzunlugu 7'den fazla karakterde";
}

private static void kontrolChar7Az(List<String> yemahhh) {
    yemahhh
            .stream()
            .allMatch(t -> t.length() <= 7);//tum elemanlar kontrol edilir hepsi sarti sagliyorsa true doner
    if (yemahhh.stream().allMatch(t -> t.length() <= 7)) {
        System.out.println("Yemek menunun tum elemanlarinin uzunlugu 7 veya 7'den az karakterde");
    } else {
        System.out.println("Yemek menunun en az bir elemaninin uzunlugu 7'den fazla karakterde");
    }
}
}